...
Now, a Levene's test is used to check if variance homogeneity is given. If the p-value reported by the Levene's test is greater than .05, the variance is homogeneous and thus acceptable.

```{r anova5, echo=T, eval = T, message=FALSE, warning=FALSE}
leveneTest(errors, group, center = median)
```

As the p-value reported by the Levene's test is greater than .05, we can proceed with the ANOVA. 
...
If Levene's test is significant, i.e. in case we are dealing with unequal variances, we can use the Welch Test which combats problems arising from violations of the homogeneity of variance assumption (Field, Miles & Field 2012: 414).
