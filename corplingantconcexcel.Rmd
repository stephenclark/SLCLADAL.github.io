...
This section exemplifies how to perform a corpus analysis using AntConc, TextPad, and Microsoft Excel. In order to follow the steps, you need to install certain pieces of software (described below).
...
AntConc is an open source concordance software which can be used to retrieve lists of key words in context (KWICs or concordances) from corpora. AntConc was developed by Laurence Anthony and can be downloaded from (his website) [http://www.laurenceanthony.net/software/antconc/] free of charge. Alternatively, type "antconc download" into your favourite search engine search box. You will then be able to download AntConc from the site which appears as the first result of your search.  The installation process is a very easy, step-by-step process.

**Installing TextPad**

To clean the concordances retrieved by AntConc, we will need a text editor. The text editor we will use is TextPad because this text editor can handle regular expressions. TextPad can be downloaded from `http://www.textpad.com/download/`. In this tutorial, we will only use the evaluation version of this program. In case you use TextPad more often, you can also purchase it for a small fee. Alternatively, type "textpad download" into your favourite search engine search box. You will then be able to download TextPad from the site which appears as the first result of your search.  The installation process is a very easy, step-by-step process.
...
In addition to the corpus data itself, we will also need metadata which contains information about the documents and/or speakers or writers whose language is represented in the corpus data.
...
Save the metadata in the workshop folder (if you have not created it already, please create a folder for today's workshop).
...
> TIP: You can watch and download short but very recommendable introductory videos on 
...
3. *seems like* across registers: search for *seems like* (an informal phrase) and *seems that* (a more formal variant) in BROWN J and BROWN A (make sure to unload the BROWN J files after you have searched them!). Based on your findings, can you guess which of the two files represents the genre "science writing" and which file represents "press reportages"?

> TIP: "press reportages" is less formal!
...
```{r echo=F, eval = F, message=FALSE, warning=FALSE}
The construction *seems that* is used more often in academic writing than by native speakers of (American) English. I did not know that *seems that* is more formal than *seems like*!
```
...
* Make a list of swear words and save this list as a `.txt` file in your project folder. IMPORTANT: Each word (or sequence) must be on its own line.

* Before starting the search, go to the main menu under `Tool Preferences` on `Concordance` and click on `Put delimiter around hits in KWIC display`. As a result, the previous and following context around the hits in Excel will appear in columns other than the hits themselves, and you will be able to edit the data with much more ease later on.

* Use AntConc to find these swear words in the ICE Ireland corpus by going to `Advanced` and finding the box `Use search term (s) from list below`. Click on it, then browse to your list of swear words, then click on `Apply` and then on `Start` under the search window. 
...
* Now look again for the swear words in your list, but now increase the context and enter the maximum value (1,000).
...
* Uncheck the `Regular expression` box, then search for `  ` (two white spaces) and replace this sequence with ` ` (a single white space). Repeat this until `TextPad` indicates that no occurrences of `  ` (two white spaces) were found.

* Click on the checkbox `Regular expression` again and then search for `\n ` (with a space after the "n"") and replace this sequence with `\n` (no space after "n"). Repeat this until `TextPad` indicates that no occurrences of `\n ` (with a space after the "n"") were found.

* Then search for `\n \t` and replace this sequence with ` ` (a single space). Repeat this until TextPad indicates that no occurrences of `\n \t` were found.

* Now highlight the data in TextPad ("Ctrl" + "a"), copy it ("Ctrl" + "c"), and paste it into a spreadsheet in Microsoft Excel ("Ctrl" + "v") in cell A2. Delete empty or unnecessary columns and give the remaining columns the headings *id*, *pre*, *token* and *post*.
...
* Highlight the *spk* column (at the top of the letter) again and under `Search and Select` click on `Reset`. In the pop-up window, under `Look for` type `> *`, and under `Replace with` nothing. Then click on `Replace all`.

* You will see that we have now extracted the speaker for a lot of swear words. To identify the remaining speakers, select the entire worksheet (click on 1 and next to A) and then go to `Customized Sorting` under `Sort and Filter`. Click the checkbox `Data have headers` and then sort the data in ascending order to *spk*. This way, all cases that do not have a unique speaker appear at the top of your spreadsheet.

* Before you manually search for the speakers, you should encode the data according to whether the particular hit is really a swear word or just a false hit. You do this by going into an empty column, labelling it with *hit* and then, if the hit is a swear word, enter a 1. If it is not a swear word, enter a 0.

Unfortunately, you must manually search the missing speakers who have used a swear word by copying a few words (they must be whole, complete words) from the previous context of the hit (i.e. from the cell next to the relevant token in the *pre* column), and look for this sequence in the ICE Ireland data using `AntConc`. Then click on the hit in the `AntConc` Concordance window (this brings you to the location in the file where the hit is) and look for the speaker, which you then insert manually into the Excel spreadsheet. 
...
* You now have a spreadsheet with the file containing the swear word, the swear word itself, the context in which it is used, and the speaker who said it. In the next step we will determine if and how often a speaker has used swear words.
...
* Now name the next two columns *sum* and *test*, and type the following command in the first cell of the column *sum*:
...
* To be able to display the text types, you must copy the *text.id* column to an empty column. Then replace the headline with *texttype*. Then, highlight this new column, and go to `Sort and Select` on `Replace`. In the window that appears, please enter `-*` and then click on `Replace all`.
...
* In the first cell of *swear*, enter the following command (assuming that the relative frequencies of the swear words are listed in column `D`): 
`= IF(D2=0; 1; 0)`
...
* In the opening window, drag ´sex´ into the `rows` area and `swear` into the `values` area. Now drag `sex` from above also into the `values` area and click `OK`.

* In the next step, copy the table and paste it back in again as values, that is: after you have copied the table, go to a free field, make a right mouse click, give on `Paste content` and select `insert values`.

* Next to the cell containing the number of female speakers, subtract the number of women who used swear words from the total number of women (262-91 = 171). Do the same for the men. Copy the table and paste it as a table of values. Now delete the column that contains the total number of speakers by gender - it is irrelevant to the way you proceed.
...
Before you begin, copy the *INNSBRUC* corpus and load the copied version into `TextPad`. Click on `F8`, select `All documents` and replace all special characters and then all double spaces with nothing. Then save the results by clicking `File` and then `Save all`.
