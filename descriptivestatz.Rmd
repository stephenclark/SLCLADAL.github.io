...
In addition, certain *packages* need to be installed from an R *library* so that the scripts shown below are executed without errors. Before turning to the code below, please install the packages needed for running the code below. If you have already installed the packages mentioned below, then you can skip ahead and ignore this section. To install the necessary packages, simply run the following code - it may take some time (between 1 and 5 minutes to install all of the packages so you do not need to worry if it takes some time).
...
Here is an example to illustrate the mode. Consider you are interested in where most speakers in the private dialogue section of the Irish component of the *International Corpus of English* are currently residing and you get the following distribution.
...
The tabulated and visualized data show that the mode is *"Dublin (City)"*, because the largest group (110 speakers) of speakers in the corpus are speakers from the city of Dublin. This means that the "average" speaker in the private dialogue section of the Irish component of the *International Corpus of English* (ICE) is from Dublin city.
...
To extract the confidence intervals for grouped data, we can use the "summarySE" function from the "Rmisc" package.
...
Confidence intervals for means by bootstrap with boot package
...
The advantage of using the bootstrapping method lies in the fact that the data is (frequently) not distributed normally which is not an issue for the bootstrapping and it will thus provide more reliable results as it does not rely on distributional assumptions about the data. 
...
We use the "MultinomCI" function to extract the confidence intervals from multinominal data.
