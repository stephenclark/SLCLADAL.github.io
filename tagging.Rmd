...
Part-of-speech tagging, or pos-tagging, is a common procedure when working with natural language data. Despite being used quite frequently, it is a rather complex issue that requires the application of statistical methods that are quite advanced. In the following, we will explore different options for pos-tagging and syntactic parsing. 
...
Once you have installed "R", "R-Studio", "Tinn-R", and have also initiated the session by executing the code shown above, you may be good to go.
...
We extract proper nouns (tag NNP for singular and tag NNPS for plural proper nouns) from paragraphs of the president's speeches.
...
>
> Merging tokens by identical consecutive POS-tags can be a useful approach to identification of multi-word-units (MWU). Modify the function `annotateDocuments` in such a way that merges consecutive POS-tags into a single token (e.g. "United_NNP States_NNP" becomes "United_States_NNP"). 
>
