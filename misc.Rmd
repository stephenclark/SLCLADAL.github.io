...
The Shapiro-Wilk test shows that the distribution in the data differs significantly from a normal distribution. However, the Shapiro-Wilk test has been found to be too generous with small samples and too harsh when dealing with large samples. For this reason, it is recommended that the test should always be accompanied by additional eye-balling of the data.
...
A variable - at least statistically speaking - is a phenomenon or feature that can take more than one state or value. If it only represents a single value, it is called a constant - because it does not vary. In order to determine whether something is in state A or state B, we have to define criteria according to which we categorize the states. For instance, we can label a state as "tall" if someone is taller than 175cm or "short" if a person is shorter than 160cm and as intermediate if someone does neither fall into the "tall" nor the "short" category. In other words, we have to define the state of a variable or make it "measurable". This procedure of making something "measurable" is called operationalization. 
...
The equivalence between variables and the concept that they operationalize is called *intrinsic validity*. An example for intrinsic validity would be an operationalization of linguistic complexity that allows us to compare entities in terms of their inherent complexity. Depending on the context, linguistic complexity may be measured or operationalized, for instance, as the number of phonemes, syllables, morphemes or syntactic nodes in a phrase, or even amalgamations of these measures.  

In contrast, *external validity* refers to the potential of the results of a study or an experiment to be applied to reality. To elaborate, we could determine, in an experimental setting, which methods causes participants to improved memory and recall of nonsense syllables (syllables that do not exist in the language(s) spoken by the participants). In this context, external validity would mean that we could transfer the findings about memory and recall of nonsense syllables to other memory and recall tasks in everyday life (for instance remembering shopping lists or learning vocabulary for another language). 
