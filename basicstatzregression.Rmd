...
In other words, to estimate how much someone who is 180cm tall weighs, we would multiply the coefficent (slope of the line) with 180 ($x$) and add the value of the intercept (point where line crosses the $y$-axis). 
...
The F- and p-values are exactly those reported by the summary which shows where the F-values comes from and what it means; namely it denotes the difference between the base-line and the more saturated model.


The degrees of freedom associated with the residual standard error are the number of cases in the model minus the number of predictors (including the intercept). The residual standard error is square root of the sum of the squared residuals of the model divided by the degrees of freedom. Have a look at the following to clear this up:
...
The model summary reports that Group B performed significantly worse (Pr(>|t|) is smaller than .001, as indicated by the three \* after the p-values) compared with Group A (the Estimate is negative). We will now create the diagnostic graphics.
