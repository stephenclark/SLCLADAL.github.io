...
Prepare data by removing all lists from the current work space and installing packages we need or that may be useful
\begin{lstlisting}
rm(list=ls(all=T))
...
\textbf{Die Linke} rallied on what has to be done ("müssen"), and used words like "sozial", "beschäftigten", and "öffentlich" a lot showing their emphasis on economic issues.
...
Finally, the \textbf{CDU/CSU} mentioned "ländlich", "wohlstand", "unser*, "and "weiterhin" to suggest that they will just continue with whatever nothing that have been doing over the past years.
